...
function scheduleSave() { $('#saveStatus').innerHTML='<i style="background:#d49b43"></i> 保存中…'; clearTimeout(saveTimer); saveTimer=setTimeout(()=>{persist();saveRecoveryDraft();},650); }
...
function scheduleSourceMapBuild(revisionAtRender){cancelSourceMapBuild();const run=()=>{sourceMapIdle=null;if(editorRevision!==revisionAtRender||previewRevision!==revisionAtRender)return;buildSourceMap();mappedRevision=revisionAtRender;$('#compileStatus').textContent='即时排版 · 双向定位';};if('requestIdleCallback' in window)sourceMapIdle=requestIdleCallback(run,{timeout:320});else sourceMapTimer=setTimeout(run,40);}
function update(recordHistory=true) {
  if(recordHistory)recordEditorHistory();
  editorRevision++;
  const lines=editor.value.split('\n');if(lines.length!==renderedLineCount){renderedLineCount=lines.length;$('#lineNumbers').textContent=lines.map((_,i)=>i+1).join('\n');$('#lineCount').textContent=`${lines.length} 行`;}
  clearTimeout(renderTimer);cancelSourceMapBuild();$('#compileStatus').textContent='正在排版…';
  renderTimer=setTimeout(()=>{const sourceAtRender=editor.value,revisionAtRender=editorRevision;previewTextCache=new WeakMap();preview.innerHTML=compile(sourceAtRender);previewRevision=revisionAtRender;mappedRevision=-1;$('#compileStatus').textContent='即时排版 · 正在建立定位…';scheduleSourceMapBuild(revisionAtRender);},160);
  scheduleSave();
}
editor.addEventListener('input',e=>{recordEditorHistory({coalesce:true,inputType:e.inputType||'input'});update(false);});editor.addEventListener('select',updateEditorHistorySelection);titleInput.addEventListener('input',scheduleSave);
...
function ensureSourceMap(){if(previewRevision!==editorRevision)return false;if(preview.children.length&&mappedRevision!==previewRevision){cancelSourceMapBuild();buildSourceMap();mappedRevision=previewRevision;$('#compileStatus').textContent='即时排版 · 双向定位';}return true;}
...
function renderLatestPreview(){if(previewRevision===editorRevision)return;clearTimeout(renderTimer);cancelSourceMapBuild();previewTextCache=new WeakMap();preview.innerHTML=compile(editor.value);previewRevision=editorRevision;mappedRevision=-1;}
function preparePrint(){renderLatestPreview();clearPreviewTextHighlights();preview.querySelectorAll('.source-sync-highlight').forEach(node=>node.classList.remove('source-sync-highlight'));}
$('#printBtn').onclick=()=>{preparePrint();requestAnimationFrame(()=>window.print());};window.addEventListener('beforeprint',preparePrint);window.addEventListener('afterprint',()=>scheduleSourceMapBuild(previewRevision));
...
function latexInlineToMarkdown(value){return value.replace(/\\colorbox\{[^}]+\}\{(?:\\strut\s*)?([^{}]*)\}/g,'**$1**').replace(/\\textcolor\{[^}]+\}\{([^{}]*)\}/g,'$1').replace(/\\textbf\{([^{}]*)\}/g,'**$1**').replace(/\\textit\{([^{}]*)\}|\\emph\{([^{}]*)\}/g,'*$1$2*').replace(/\\texttt\{([^{}]*)\}/g,'`$1`').replace(/\\href\{([^{}]+)\}\{([^{}]+)\}/g,'[$2]($1)').replace(/\\([_%&#$])/g,'$1');}
function latexTableToZhihuMarkdown(block){const content=block.match(/\\begin\{tabularx?\}[^\n]*\n([\s\S]*?)\\end\{tabularx?\}/)?.[1];if(!content)return '\n> [复杂表格请参考原 LaTeX 文档]\n';const hasHeader=/\\midrule\b/.test(content),rows=content.replace(/\\(?:toprule|midrule|bottomrule|hline)\b/g,'').split(/\\\\/).map(row=>row.trim()).filter(Boolean).map(row=>row.split('&').map(cell=>latexInlineToMarkdown(cell.trim()).replace(/\|/g,'\\|')));if(!rows.length)return '';const width=Math.max(...rows.map(row=>row.length)),header=hasHeader?rows.shift():Array.from({length:width},(_,i)=>`列 ${i+1}`),caption=block.match(/\\caption\{([^}]*)\}/)?.[1]||'',line=row=>`| ${Array.from({length:width},(_,i)=>row[i]||'').join(' | ')} |`;return `\n${caption?`**表：${latexInlineToMarkdown(caption)}**\n\n`:''}${line(header)}\n${line(Array(width).fill('---'))}\n${rows.map(line).join('\n')}\n`;}
function latexToZhihuMarkdown(source){let body=source.replace(/^[\s\S]*?\\begin\{document\}/,'').replace(/\\end\{document\}[\s\S]*$/,'').replace(/\\maketitle|\\tableofcontents|\\(?:newpage|clearpage|pagebreak)\b/g,''),blocks=[];const hold=value=>{blocks.push(value);return `@@ZHIHUBLOCK${blocks.length-1}@@`;},equationLabels=new Map();let equationNumber=0;
  body=body.replace(/\\begin\{(?:verbatim|lstlisting)\}([\s\S]*?)\\end\{(?:verbatim|lstlisting)\}/g,(_,code)=>hold(`\n\`\`\`\n${code.trim()}\n\`\`\`\n`));
  body=body.replace(/\\begin\{equation(\*)?\}([\s\S]*?)\\end\{equation\1\}/g,(_,star,value)=>{const label=value.match(/\\label\{([^}]+)\}/)?.[1],math=value.replace(/\\label\{[^}]+\}/g,'').trim();if(!star){equationNumber++;if(label)equationLabels.set(label,equationNumber);}return hold(`\n$$\n${math}\n$$${star?'':`  (${equationNumber})`}\n`);});
  body=body.replace(/\\\[([\s\S]*?)\\\]|\$\$([\s\S]*?)\$\$/g,(_,a,b)=>hold(`\n$$\n${(a||b).trim()}\n$$\n`));
  body=body.replace(/\\begin\{table\}(?:\[[^\]]*\])?([\s\S]*?)\\end\{table\}/g,match=>hold(latexTableToZhihuMarkdown(match)));
  body=body.replace(/\\begin\{figure\}(?:\[[^\]]*\])?([\s\S]*?)\\end\{figure\}/g,(_,value)=>{const name=value.match(/\\includegraphics(?:\[[^\]]*\])?\{([^}]+)\}/)?.[1]||'图片',caption=value.match(/\\caption\{([^}]+)\}/)?.[1]||'';return hold(`\n[请在知乎确认图片：${name}]${caption?`\n*${latexInlineToMarkdown(caption)}*`:''}\n`);});
  const boxNames={defbox:'定义',thmbox:'定理',exbox:'例题',whybox:'直觉',sumbox:'总结'};body=body.replace(/\\begin\{(defbox|thmbox|exbox|whybox|sumbox)\}(?:\[([^\]]*)\])?([\s\S]*?)\\end\{\1\}/g,(_,type,label,value)=>`\n> **${boxNames[type]}${label?'：'+label:''}**\n> ${value.trim().replace(/\n/g,'\n> ')}\n`).replace(/\\begin\{(theorem|proof)\}([\s\S]*?)\\end\{\1\}/g,(_,type,value)=>`\n> **${type==='proof'?'证明':'定理'}** ${value.trim()}\n`);
  const headingMarks={section:'##',subsection:'###',subsubsection:'####',paragraph:'#####',subparagraph:'######'};body=body.replace(/\\(section|subsection|subsubsection|paragraph|subparagraph)\*?\{([^}]*)\}/g,(_,command,text)=>`\n${headingMarks[command]} ${latexInlineToMarkdown(text)}\n`).replace(/\\begin\{itemize\}([\s\S]*?)\\end\{itemize\}/g,(_,value)=>value.replace(/\\item\s*/g,'- ')).replace(/\\begin\{enumerate\}([\s\S]*?)\\end\{enumerate\}/g,(_,value)=>{let index=0;return value.replace(/\\item\s*/g,()=>`${++index}. `);});
  body=latexInlineToMarkdown(body).replace(/\\eqref\{([^}]+)\}/g,(_,label)=>`(${equationLabels.get(label)??'?'})`).replace(/\\ref\{([^}]+)\}/g,(_,label)=>String(equationLabels.get(label)??'?')).replace(/\\(centering|hrulefill)\b|\\label\{[^}]+\}/g,'').replace(/^\s*%.*$/gm,'').replace(/\\\\/g,'\n');
  body=body.replace(/@@ZHIHUBLOCK(\d+)@@/g,(_,index)=>blocks[Number(index)]);return body.replace(/\n{3,}/g,'\n\n').trim();}
function buildZhihuClipboard(){renderLatestPreview();clearPreviewTextHighlights();const clone=preview.cloneNode(true),originalElements=[...preview.querySelectorAll('*')],cloneElements=[...clone.querySelectorAll('*')],properties=['display','color','background-color','font-family','font-size','font-weight','font-style','line-height','text-align','text-decoration','vertical-align','white-space','border','border-top','border-right','border-bottom','border-left','border-collapse','border-spacing','border-radius','padding','padding-top','padding-right','padding-bottom','padding-left','margin','margin-top','margin-right','margin-bottom','margin-left','width','max-width','height','min-height','position','top','left','right','bottom'];for(let i=0;i<Math.min(originalElements.length,cloneElements.length);i++){const computed=getComputedStyle(originalElements[i]),target=cloneElements[i];for(const property of properties){const value=computed.getPropertyValue(property);if(value&&value!=='normal'&&value!=='none'&&value!=='auto')target.style.setProperty(property,value);}}clone.querySelectorAll('.katex-mathml,.note-toc').forEach(node=>node.remove());clone.querySelector('h1')?.remove();clone.querySelector('.author')?.remove();for(const target of clone.querySelectorAll('*'))for(const attribute of [...target.attributes])if(attribute.name==='class'||attribute.name==='id'||attribute.name==='title'||attribute.name.startsWith('data-source-'))target.removeAttribute(attribute.name);const title=`<h1 style="font-size:28px;line-height:1.35;margin:0 0 24px;font-weight:700;color:#111">${escapeHtml(titleInput.value||'未命名文章')}</h1>`,html=`<article>${title}${clone.innerHTML}</article>`,plain=`# ${titleInput.value||'未命名文章'}\n\n${latexToZhihuMarkdown(editor.value)}`;return {html,plain};}
...
window.addEventListener('folio:dependency-ready',()=>{update(false);updateCustomFormula();if(formulaDialog.open)drawFormulaGuide();});
