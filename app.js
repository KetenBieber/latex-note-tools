...
function latexToZhihuMarkdown(source){let body=source.replace(/^[\s\S]*?\\begin\{document\}/,'').replace(/\\end\{document\}[\s\S]*$/,'').replace(/\\maketitle|\\tableofcontents|\\(?:newpage|clearpage|pagebreak)\b/g,''),blocks=[],references=collectLatexReferences(source);const hold=value=>{blocks.push(value);return `@@ZHIHUBLOCK${blocks.length-1}@@`;};let equationNumber=0;
  ...
  body=body.replace(/@@ZHIHUBLOCK(\d+)@@/g,(_,index)=>blocks[Number(index)]);return body.replace(/[ \t]+$/gm,'').replace(/\n{3,}/g,'\n\n').trim();}
function zhihuMarkdownDocument(){return `# ${latexInlineToMarkdown(titleInput.value||'未命名文章')}\n\n${latexToZhihuMarkdown(editor.value)}\n`;}
...
$('#downloadZhihuMd').onclick=()=>{downloadText(zhihuMarkdownDocument(),(titleInput.value||'知乎文章').replace(/[\\/:*?"<>|]/g,'-')+'.md','text/markdown;charset=utf-8');toast('已下载知乎兼容 Markdown');};
