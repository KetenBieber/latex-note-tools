...
function compile(source) {
  ...
  const equationLabels=new Map();let equationCounter=0,equationStarCounter=0;
  ...
  body=body.replace(/\\begin\{equation(\*)?\}([\s\S]*?)\\end\{equation\1\}/g,(_,star,x)=>{const numbered=!star,number=numbered?++equationCounter:null,label=x.match(/\\label\{([^}]+)\}/)?.[1]||'',math=x.replace(/\\label\{[^}]+\}/g,'').trim(),serial=numbered?number:`star-${++equationStarCounter}`,id=label?`equation-${label.replace(/[^\w:-]+/g,'-')}`:`equation-${serial}`;if(label&&numbered)equationLabels.set(label,number);return `<div class="display-equation${numbered?'':' unnumbered'}" id="${escapeHtml(id)}"><span class="equation-math">${renderMath(math,true)}</span>${numbered?`<span class="equation-number">(${number})</span>`:''}</div>`;});
  body=body.replace(/\\eqref\{([^}]+)\}/g,(_,label)=>`(${equationLabels.get(label)??'?'})`).replace(/\\ref\{([^}]+)\}/g,(_,label)=>String(equationLabels.get(label)??'?'));
