...
- 每次输入停止约 0.65 秒后，正文会写入浏览器 IndexedDB 恢复区；图片只在新增、导入或清空时单独重写，避免每次按键都复制大图片。
...
- 750 ms 内的连续键入合并为一个撤销批次，避免为每个字符长期保留一份完整文档。
- 撤销历史会根据文档大小动态限制数量，长文档不会无限占用内存。
- 快速预览与双向定位索引分开调度：先显示排版结果，再在浏览器空闲时建立定位范围。
- 普通段落使用直接文本索引；只有无法精确命中的复杂结构才进入上下文模糊匹配。
